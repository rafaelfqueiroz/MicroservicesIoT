...
public class NoCBRequestService implements RemoteRequestService  {
	...
	public Double getTemperatureFromSensor(String sensorPort) {
		...
		try {
			temperature = restTemplate.getForObject("http://localhost:"+sensorPort+"/temperature/now", Double.class, sensorPort);
		} catch (Exception ex) {
	...
	public Double getTemperatureFromSensor(String sensorPort, Integer time) {
		Double temperature = null;
		try {
			temperature = restTemplate.getForObject("http://localhost:"+sensorPort+"/temperature/"+time, Double.class, sensorPort);
		} catch (Exception ex) {
			System.out.println("[" + formatadorData.format(new Date()) + "] " +  ex.getMessage());
			temperature = 0.0;
		}
